...
def main():
    ...
    try:
        ...
        cam_bp = None
        cam_bp = world.get_blueprint_library().find('sensor.lidar.ray_cast')'
        cam_bp.set_attribute("channels",str(32))
        cam_bp.set_attribute("range",str(100.0*200))
        cam_bp.set_attribute("lower_fov",str(-30))
        cam_bp.set_attribute("upper_fov",str(15))

        cam_bp.set_attribute("points_per_second",str(56000*10))
        cam_bp.set_attribute("rotation_frequency",str(40))

        cam_location = carla.Location(0,0,2)
        cam_rotation = carla.Rotation(0,0,0)
        cam_transform = carla.Transform(cam_location,cam_rotation)
        ego_cam = world.spawn_actor(cam_bp,cam_transform,attach_to=ego_vehicle, attachment_type=carla.AttachmentType.Rigid)
        ego_cam.listen(lambda point_cloud: point_cloud.save_to_disk('Dataset/new_lidar_output/%.6d.jpg' % point_cloud.frame))
